...
| JDK 组 | Boot 线 | 逻辑定位 |
|---|---|---|
| JDK 8 | `2.3.x`、`2.4.x`、`2.5.x`、`2.6.x`、`2.7.x` | Spring Boot 2 历史兼容组 |
| JDK 17 | `3.0.x`、`3.1.x`、`3.2.x`、`3.3.x`、`3.4.x`、`3.5.x` | 当前主能力收敛组 |
...
能力契约是唯一逻辑事实源。每个分支实现一个适配层：JDK 8 使用 `javax` 与 Spring Boot 2 兼容机制；JDK 17 使用 Boot 3 的对应机制；JDK 21 使用 Boot 4 与最新 JDK API。适配层不得改变同一契约的成功结果、失败语义、默认值、回退策略和资源清理责任。
...
2. **JDK 8 组**：以 `2.3.x` 到 `2.7.x` 的 Boot 2 差异实现共同契约，保留 Java 8。
3. **JDK 17 组**：校验 `3.0.x` 到 `3.5.x` 的逻辑连续性，并处理各 Boot 3 版本的注册/Jakarta 差异。
