...
public class QLExpressConfig {
    ...
    public Object expressRunner() {
        try {
            // 使用反射创建Express4Runner，避免编译时依赖
            Class<?> runnerClass = Class.forName("com.alibaba.qlexpress4.Express4Runner");
            Object runner = runnerClass.getDeclaredConstructor().newInstance();
            
            // 注意：自定义函数需要根据qlexpress4的API来实现
            // 如果需要注册自定义函数，请使用qlexpress4的新API
    ...
    public Object expressContext() {
        try {
            Class<?> contextClass = Class.forName("com.alibaba.qlexpress4.runtime.context.MapExpressContext");
