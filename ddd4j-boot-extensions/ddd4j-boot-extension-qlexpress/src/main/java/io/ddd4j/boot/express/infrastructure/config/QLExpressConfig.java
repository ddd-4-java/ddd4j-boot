...
/**
 * QLExpress Spring 配置类。
 *
 * <p>从 ddd4j-extension-qlexpress 迁入 ddd4j-boot-extension-qlexpress：
 * 该类强依赖 Spring（@Configuration / @Bean / ObjectProvider / AnnotationUtils），
 * 属于 Spring 框架适配层职责，不应放在通用 ddd4j 库中。
 *
 * <p>负责初始化 QLExpress 引擎并注册自定义函数。
 *
 * <p>函数注册策略：
 * <ol>
 *   <li>硬编码格式的函数：放置在 ddd4j-extension-qlexpress 的 infrastructure.function 目录，逻辑固定</li>
...
public class QLExpressConfig {
     ...
     * 注册硬编码格式的函数
     *
     * <p>这些函数逻辑固定，放置在 ddd4j-extension-qlexpress 的 infrastructure.function 目录。
     * 包括字符串处理函数和日期处理函数等。
     *
