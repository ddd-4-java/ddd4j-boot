...
EOF
if python3 "$VERIFIER" --matrix "$TMP_DIR/matrix.tsv" --input "$TMP_DIR/missing.tsv" >/dev/null 2>&1; then
...
EOF
if python3 "$VERIFIER" --matrix "$TMP_DIR/matrix.tsv" --input "$TMP_DIR/drift.tsv" >/dev/null 2>&1; then
...
branch	github_sha	codeup_sha	java	spring_boot	ddd4j	logical_state	report_ref
2.3.x	1111111111111111111111111111111111111111	1111111111111111111111111111111111111111	8	2.3.12.RELEASE	1.0.x.20260630-SNAPSHOT	BLOCKED	report.md
EOF
