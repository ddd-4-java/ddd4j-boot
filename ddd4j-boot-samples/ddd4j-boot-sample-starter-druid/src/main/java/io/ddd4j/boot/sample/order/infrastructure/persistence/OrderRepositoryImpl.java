...
import io.ddd4j.boot.sample.order.domain.repository.OrderQuery;
import io.ddd4j.boot.sample.order.domain.repository.OrderRepository;
import io.ddd4j.boot.sample.order.infrastructure.messaging.OrderDomainEventPublisher;
    ...
    public Order save(Order order) {
        ...
        if (events != null && !events.isEmpty()) {
            ...
            for (DomainEvent event : events) {
                if (event instanceof io.ddd4j.boot.sample.order.domain.event.OrderCreatedEvent) {
                    io.ddd4j.boot.sample.order.domain.event.OrderCreatedEvent createdEvent =
                            (io.ddd4j.boot.sample.order.domain.event.OrderCreatedEvent) event;
