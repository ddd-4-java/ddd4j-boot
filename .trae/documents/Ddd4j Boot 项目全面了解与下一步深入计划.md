## 项目概述
- Ddd4j Boot 是基于 Spring Boot 3.5.x 的快速开发脚手架，聚合通用依赖、基础 API 与多种自动配置能力，支持 WebMVC 与 WebFlux 两种风格，提供丰富的演示样例与打包部署支持。
...
- WebMVC/WebFlux：本地化、主题、拦截器、全局异常处理；Jackson：序列化配置；Kafka/Sa-Token/License/Crypto/DataScope/External 等均有开箱集成
...
## 示例工程与运行
- `ddd4j-boot-samples` 提供不同数据源/消息中间件的组合示例，包含 `application*.yaml`、`log4j2*.xml`、模板与 i18n 资源，便于快速本地验证
